# NewAge Products — Returns & Claims Control Tower v6.0
...
[data-testid="stSidebar"] ::-webkit-scrollbar-track {{ background: #1E3A5F; border-radius: 3px; }}
[data-testid="stSidebar"] ::-webkit-scrollbar-thumb {{ background: #A0AEC0; border-radius: 3px; }}
[data-testid="stSidebar"] ::-webkit-scrollbar-thumb:hover {{ background: #CBD5E0; }}
...
def render_sidebar(orders, rules):
    with st.sidebar:
        ...
        if uploaded:
            ...
            st.session_state["last_uploaded"] = datetime.now().strftime("%b %d, %Y %H:%M")
            st.success("✅ New data loaded")
        else:
            st.markdown("<div style='font-size:11px; color:#718096; margin-bottom:4px;'>📂 Using default dataset</div>", unsafe_allow_html=True)

        # Last updated display
        last_up = st.session_state.get("last_uploaded", None)
        if last_up:
            st.markdown(f"""
            <div style='background:#1A4731;border-radius:4px;padding:5px 10px;margin-bottom:8px;'>
                <div style='font-size:9px;color:#9AE6B4;font-weight:700;text-transform:uppercase;'>Last Uploaded</div>
                <div style='font-size:11px;color:white;font-weight:600;'>{last_up}</div>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
            <div style='background:#2D3748;border-radius:4px;padding:5px 10px;margin-bottom:8px;'>
                <div style='font-size:9px;color:#A0AEC0;font-weight:700;text-transform:uppercase;'>Data Date</div>
                <div style='font-size:11px;color:#CBD5E0;'>Default dataset — Sept-Nov 2024</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        # -- Search --
        st.markdown("<span style='font-size:10px; font-weight:700; color:#A0AEC0; text-transform:uppercase; letter-spacing:1px;'>SEARCH</span>", unsafe_allow_html=True)
        search_term = st.text_input("", placeholder="Search order ID, SKU, carrier...",
                                     label_visibility="collapsed",
                                     key="global_search")
        if search_term:
            st.markdown("<div style='font-size:10px;color:#F6E05E;margin-bottom:4px;'>Searching: " + str(search_term) + "</div>", unsafe_allow_html=True)
        ...
        retailers_list = ["All Retailers", "Costco", "Home Depot", "Wayfair", "Amazon", "Lowe\'s", "Target", "Best Buy", "Canadian Tire", "RONA", "Leon\'s Furniture"]
        if "Customer" in orders.columns:
            customers_in_data = sorted(orders["Customer"].unique().tolist())
            retailers_list = ["All Retailers"] + customers_in_data

        sel_carrier  = st.selectbox("Carrier", carriers)
        sel_cat      = st.selectbox("Product Category", categories)
        sel_retailer = st.selectbox("Retailer / Customer", retailers_list)

        # Clear filters button
        if sel_carrier != "All Carriers" or sel_cat != "All Categories" or sel_retailer != "All Retailers" or search_term:
        ...
        st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
        st.markdown("<span style='font-size:10px;font-weight:700;color:#A0AEC0;text-transform:uppercase;letter-spacing:1px;'>MODULE GUIDE</span>", unsafe_allow_html=True)
        st.markdown(f"""<div style='margin-top:6px;'><div style='padding:5px 0;border-bottom:1px solid #2C5282;'><div style='font-size:10px;font-weight:700;color:#CBD5E0;'>📊 Executive Dashboard</div><div style='font-size:9px;color:#718096;margin-top:1px;'>KPI banner, cost leakage radar & exception heat map — your 30-second situation overview.</div></div><div style='padding:5px 0;border-bottom:1px solid #2C5282;'><div style='font-size:10px;font-weight:700;color:#CBD5E0;'>[!] Exception Queue</div><div style='font-size:9px;color:#718096;margin-top:1px;'>All flagged exceptions ranked by Recovery Opportunity Score — highest value first.</div></div><div style='padding:5px 0;border-bottom:1px solid #2C5282;'><div style='font-size:10px;font-weight:700;color:#CBD5E0;'>📋 Claim Pipeline</div><div style='font-size:9px;color:#718096;margin-top:1px;'>5-stage claim tracker (Identified → Recovered) with priority model and deadline alerts.</div></div><div style='padding:5px 0;border-bottom:1px solid #2C5282;'><div style='font-size:10px;font-weight:700;color:#CBD5E0;'>🚚 Carrier Scorecard</div><div style='font-size:9px;color:#718096;margin-top:1px;'>On-time %, invoice accuracy & damage rate per carrier — RED/YELLOW/GREEN status.</div></div><div style='padding:5px 0;border-bottom:1px solid #2C5282;'><div style='font-size:10px;font-weight:700;color:#CBD5E0;'>📦 SKU Intelligence</div><div style='font-size:9px;color:#718096;margin-top:1px;'>Return rates vs 8% threshold by SKU — flags high-risk products for QC review.</div></div><div style='padding:5px 0;border-bottom:1px solid #2C5282;'><div style='font-size:10px;font-weight:700;color:#CBD5E0;'>🤖 AI Recommendations</div><div style='font-size:9px;color:#718096;margin-top:1px;'>5 Claude-generated operational insights based on your live exception data.</div></div><div style='padding:5px 0;border-bottom:1px solid #2C5282;'><div style='font-size:10px;font-weight:700;color:#CBD5E0;'>📝 Executive Summary</div><div style='font-size:9px;color:#718096;margin-top:1px;'>Daily or weekly visual leadership briefing — charts, risks, actions & AI narrative.</div></div><div style='padding:5px 0;border-bottom:1px solid #2C5282;'><div style='font-size:10px;font-weight:700;color:#CBD5E0;'>📅 Daily Control Tower</div><div style='font-size:9px;color:#718096;margin-top:1px;'>8 daily workflow tabs: returns aging, claims, invoices, carriers, chargebacks & more.</div></div><div style='padding:5px 0;border-bottom:1px solid #2C5282;'><div style='font-size:10px;font-weight:700;color:#CBD5E0;'>💰 Financial Recovery Calculator</div><div style='font-size:9px;color:#718096;margin-top:1px;'>Full leakage breakdown — overcharges, duplicates, claims, returns & invoice variance.</div></div><div style='padding:5px 0;border-bottom:1px solid #2C5282;'><div style='font-size:10px;font-weight:700;color:#CBD5E0;'>🧮 Return Cost Calculator</div><div style='font-size:9px;color:#718096;margin-top:1px;'>15-input cost model — compares Return to DC vs Field Destruction vs Replacement Part.</div></div><div style='padding:5px 0;border-bottom:1px solid #2C5282;'><div style='font-size:10px;font-weight:700;color:#CBD5E0;'>🏪 Retailer Chargeback Hub</div><div style='font-size:9px;color:#718096;margin-top:1px;'>6-retailer dashboard with ASN logic, dispute scorer, SOP checklists & heat maps.</div></div><div style='padding:5px 0;border-bottom:1px solid #2C5282;'><div style='font-size:10px;font-weight:700;color:#CBD5E0;'>📚 Operations Playbook</div><div style='font-size:9px;color:#718096;margin-top:1px;'>Exception cheatsheets, claim checklists, SLA reference & team ownership matrix.</div></div><div style='padding:5px 0;border-bottom:1px solid #2C5282;'><div style='font-size:10px;font-weight:700;color:#CBD5E0;'>🔧 Platform Roadmap</div><div style='font-size:9px;color:#718096;margin-top:1px;'>4-phase evolution from Excel today to ERP/API integration tomorrow.</div></div></div>""", unsafe_allow_html=True)
    ...
    return nav, sel_carrier, sel_cat, sel_retailer, search_term


# -- Header ---------------------------------------------------------------------
def render_header(sel_carrier, sel_cat, sel_retailer="All Retailers", search_term=""):
    # Build active filter chips
    chips = []
    if sel_carrier  != "All Carriers":   chips.append(("🚚", sel_carrier,  "2C5282"))
    if sel_cat      != "All Categories": chips.append(("📦", sel_cat,      "744210"))
    if sel_retailer != "All Retailers":  chips.append(("🏪", sel_retailer, "1A4731"))
    if search_term:                       chips.append(("🔍", search_term,  "4A1A6B"))

    chip_html = ""
    for icon, label, color in chips:
        chip_html += (
            f"<span style=\"background:#{color};color:white;padding:3px 10px;"
            f"border-radius:12px;font-size:11px;font-weight:600;margin-right:6px;\">"
            f"{icon} {label}</span>"
        )

    filter_row = (
        f"<div style=\"margin-top:8px;\">{chip_html}</div>" if chips else ""
    )

    st.markdown(
        "<div style=\"background:linear-gradient(135deg,#17375E 0%,#2C5282 60%,#1A365D 100%);"
        "border-radius:12px;padding:0;margin-bottom:18px;overflow:hidden;"
        "box-shadow:0 4px 20px rgba(0,0,0,0.25);\">"

        # Top accent line
        "<div style=\"height:4px;background:linear-gradient(90deg,#F6AD10,#ED6C02,#D32F2F,#2E7D32,#3182CE);\">"
        "</div>"
        ...
        "🏭</div>"
        "<div>"
        "<div style=\"font-size:11px;font-weight:700;color:#F6AD10;text-transform:uppercase;"
        "letter-spacing:2px;margin-bottom:2px;\">NewAge Products</div>"
        "<div style=\"font-size:24px;font-weight:800;color:white;line-height:1.1;"
        "letter-spacing:-0.3px;\">Returns &amp; Claims Control Tower</div>"
        "<div style=\"font-size:11px;color:#CBD5E0;margin-top:4px;\">"
        "AI-Assisted Exception Management &nbsp;&middot;&nbsp; Freight Recovery &nbsp;&middot;&nbsp; Carrier Performance"
        "</div>"
        "</div></div>"
        ...
        "<div style=\"font-size:10px;color:#A0AEC0;font-weight:600;\">LIVE DASHBOARD</div>"
        f"<div style=\"font-size:13px;font-weight:700;color:white;\">{datetime.now().strftime('%b %d, %Y')}</div>"
        f"<div style=\"font-size:10px;color:#CBD5E0;\">{datetime.now().strftime('%H:%M')} &nbsp; v6.0</div>"
        "</div></div>"
        "</div>"
        ...
        # Bottom stat strip
        "<div style=\"background:rgba(0,0,0,0.2);padding:8px 28px;"
        "display:flex;gap:24px;border-top:1px solid rgba(255,255,255,0.08);\">"
        "<span style=\"font-size:11px;color:#CBD5E0;\">🔧 <strong style=\"color:white;\">12 Modules</strong></span>"
        "<span style=\"font-size:11px;color:#CBD5E0;\">🤖 <strong style=\"color:white;\">Claude AI</strong> Powered</span>"
        "<span style=\"font-size:11px;color:#CBD5E0;\">📊 <strong style=\"color:white;\">8 Rules</strong> Engine</span>"
        "<span style=\"font-size:11px;color:#CBD5E0;\">🏪 <strong style=\"color:white;\">6 Retailers</strong> Tracked</span>"
        "<span style=\"font-size:11px;color:#CBD5E0;\">📅 <strong style=\"color:white;\">Daily</strong> Operations</span>"
        "</div>"
        "</div>",
        unsafe_allow_html=True
    )
...
def main():
    ...
    nav, sel_carrier, sel_cat, sel_retailer, search_term = render_sidebar(orders, rules_placeholder := {})

    orders_f   = orders.copy()
    invoices_f = invoices.copy()
    tracking_f = tracking.copy()
    ...
    if sel_carrier != "All Carriers":
        ...
        tracking_f = tracking_f[tracking_f["Carrier"] == sel_carrier]
        claims_f   = claims_f[claims_f["Carrier"] == sel_carrier]

    if sel_cat != "All Categories":
        orders_f = orders_f[orders_f["Product Category"] == sel_cat]

    if sel_retailer != "All Retailers" and "Customer" in orders_f.columns:
        orders_f = orders_f[orders_f["Customer"] == sel_retailer]

    if search_term:
        mask = pd.Series([False] * len(orders_f))
        for col in ["Order ID", "SKU", "Carrier", "Tracking Number", "Customer", "Product Name"]:
            if col in orders_f.columns:
                mask = mask | orders_f[col].astype(str).str.contains(search_term, case=False, na=False)
        orders_f = orders_f[mask]

    # Show filter active warning in sidebar
    if sel_carrier != "All Carriers" or sel_cat != "All Categories" or sel_retailer != "All Retailers" or search_term:
        filter_parts = []
        if sel_carrier  != "All Carriers":   filter_parts.append(sel_carrier)
        if sel_cat      != "All Categories": filter_parts.append(sel_cat)
        if sel_retailer != "All Retailers":  filter_parts.append(sel_retailer)
        if search_term:                       filter_parts.append(f'Search: "{search_term}"')
        st.sidebar.markdown(f"""
        <div style='background:#744210;border-radius:6px;padding:8px 12px;margin:8px 0;'>
            <div style='font-size:10px;font-weight:700;color:#F6E05E;text-transform:uppercase;'>Filter Active</div>
            <div style='font-size:11px;color:white;margin-top:2px;'>{" + ".join(filter_parts)}</div>
            <div style='font-size:9px;color:#FBD38D;margin-top:2px;'>All views filtered to this selection</div>
        </div>
        """, unsafe_allow_html=True)

    # Run logic
    rules              = run_business_rules(orders_f, invoices_f, tracking_f, claims_f)
    claims_prioritized = prioritize_claims(claims_f)
    scorecard          = carrier_scorecard(orders_f, claims_f, invoices_f)

    render_header(sel_carrier, sel_cat, sel_retailer, search_term)
...
def render_chargeback_hub():
    ...
    with tabs[0]:
        ...
        fig.update_layout(
            ...
            title=dict(text="Chargeback Exposure by Retailer", font=dict(size=11,color=SLATE)),
        )
        st.plotly_chart(fig, use_container_width=True)
